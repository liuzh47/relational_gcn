...
from dataset import WN11Dataset
...
def calc_mrr(
    emb, w, test_mask, triplets_to_filter, batch_size=100, filter=True, hits=[1,3,10]
):
    with torch.no_grad():
        ...
        hits_res = []
        for hit in hits:
            avg_cnt = torch.mean((ranks <= hit).float())
            hits_res.append(avg_cnt)
    return mrr, hits_res
...
def train(
    ...
    for epoch, batch_data in enumerate(dataloader):  # single graph batch
        ...
        optimizer.step()
        print(
            "Epoch {:04d} | Loss {:.4f} | Best MRR {:.4f} | Best Hits 1 {:.4f} | Best Hits 3 {:.4f} | Best Hits 10 {:.4f}".format(
                epoch, loss.item(), best_mrr, best_hit_1, best_hit_3, best_hit_10
            )
        )
        if (epoch + 1) % 500 == 0:
            ...
            if best_hit_1 < hits_res[0]:
                best_hit_1 = hits_res[0]
            if best_hit_3 < hits_res[1]:
                best_hit_3 = hits_res[1]
            if best_hit_10 < hits_res[2]:
                best_hit_10 = hits_res[2]
...
if __name__ == "__main__":
    ...
    data = WN11Dataset(reverse=False)
    g = data[0]
    ...
    embed = model(test_g, test_nids)
    best_mrr, hits_res = calc_mrr(
        embed, model.w_relation, test_mask, triplets, batch_size=500
    )
    print(
        "Best MRR {:.4f}  | Best Hits 1 {:.4f} | Best Hits 3 {:.4f} | Best Hits 10 {:.4f} achieved using the epoch {:04d} ".format(
            best_mrr, hits_res[0], hits_res[1], hits_res[2], checkpoint["epoch"]
        )
    )
